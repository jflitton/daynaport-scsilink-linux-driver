 ...
 *   - All SCSI I/O goes through one persistent Scsi_Request per interface
 *     (scsi_allocate_request).  The adapter handles one command at a time, so a
 *     `busy` flag (under a spinlock) serializes RX and TX onto that single
 *     request -- exactly the one-outstanding-command model the hardware wants.
 *   - There is no RX interrupt, so receive is polled: a timer issues READ(6) via
 *     the asynchronous scsi_do_req(); its completion callback delivers packets
 *     and re-arms the timer at an adaptive rate.  TX uses the same async path;
 ...
 * can return more than we ask for.  scsilink_rx_kick() zeroes this WHOLE buffer
 ...
 * idle rate.
 *
 * The three knobs are module parameters (milliseconds), so the cadence can be
 * swept at load time without rebuilding:
 *
 ...
 * scsilink_init, each floored at 1 tick.  The tick variables carry HZ-derived
 * defaults so a non-module (built-in) build is still sane. */
...
/* RX poll timer: issue a READ(6).  Timer (softirq) context - must not sleep. */
static void scsilink_rx_kick(unsigned long arg)
{
	...
	if (!dp->running || dp->sreq == NULL)
		return;

	/* The adapter handles one command at a time.  If the single request is
	 * busy here it is a TX in flight (an RX in flight means the timer is not
	 * pending) -- retry at the fast rate so RX does not stall behind a TX. */
	 ...
	 * to stop (we do not trust the device's flag field for that).  Zero the
	 * ENTIRE buffer, not just rx_req_len: ZuluSCSI ignores the requested length,
	 * so a larger batch would otherwise run past the cleared region into stale
	 * bytes that parse as a bogus record. */
...
/* READ(6) completion - SCSI done context.  Deliver packets, re-arm poll. */
static void scsilink_recv_done(Scsi_Cmnd *SCpnt)
{
	 ...
	 * which the HBA reports as a short transfer (non-DID_OK) even though the
	 * data is fine -- so we trust the self-describing, length-checked buffer
	 * rather than the SCSI result and parse regardless. */
	n = scsilink_rx(dp);
	...
	/* Re-arm the poll at the rate the shared cadence policy picks: fast while
	 * to-us frames are arriving and through a brief hold afterwards, idle once
	 * that hold is spent. */
	...
	/* This RX freed the single command block; let a pushed-back TX retry. */
...
static int scsilink_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* One command at a time: if the request is busy (RX poll or a prior TX in
	 * flight) stop the queue and ask the stack to requeue; recv_done/send_done
	 * wake it when the block frees. */
	spin_lock_irqsave(&dp->lock, flags);
	if (dp->busy) {
...
static void scsilink_send_done(Scsi_Cmnd *SCpnt)
{
	...
	/* The single command block is free again; a pending RX timer will poll, and
	 * the stack can hand us the next frame. */
	if (dp->running)
		netif_wake_queue(dp->dev);
}
...
static int scsilink_stop(struct net_device *dev)
{
	 ...
	 * will not re-arm.  Wait it out so nothing touches dp after we return. */
	scsilink_drain(dp);
 ...
 * so this is a backstop: log, count, and kick the queue. */
